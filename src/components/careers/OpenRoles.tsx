...
function RoleRow({ title, department, status, isLast, delay, onClick, isMobile }: { title: string; department: string; status: string; isLast: boolean; delay: number; onClick: () => void; isMobile: boolean }) {
...
function WildcardCard({ isMobile }: { isMobile: boolean }) {
  ...
  return (
    <div ref={ref} style={{ background: '#000000', borderRadius: 20, padding: isMobile ? '40px 28px' : '52px 56px', marginTop: 20, display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'flex-start' : 'center', justifyContent: 'space-between', gap: isMobile ? 28 : 48, opacity: isVisible ? 1 : 0, transform: isVisible ? 'translateY(0)' : 'translateY(28px)', transition: 'opacity 650ms ease, transform 650ms ease' }}>
      <div>
        <div style={{ fontFamily: "'Open Sauce One', sans-serif", fontWeight: 700, fontSize: 11, color: '#FFBC3B', letterSpacing: '0.16em', textTransform: 'uppercase', marginBottom: 14 }}>Always Open</div>
        <div style={{ fontFamily: "'Open Sauce One', sans-serif", fontWeight: 800, fontSize: isMobile ? 22 : 26, color: '#FFFFFF', marginBottom: 14 }}>Do not see your role here?</div>
        <div style={{ fontFamily: "'Open Sauce One', sans-serif", fontWeight: 400, fontSize: 15, color: 'rgba(255,255,255,0.42)', maxWidth: 520, lineHeight: 1.7 }}>Everyone here wants to be the world's best at what they do. If that is you and you have a skill that belongs at the Forge, write to us directly. We read every email. If the fit is there, we find a way.</div>
      </div>
      <a href={TALLY_FORM} target="_blank" rel="noopener noreferrer"
        style={{ background: '#FFBC3B', color: '#000000', borderRadius: 100, padding: '15px 32px', fontFamily: "'Open Sauce One', sans-serif", fontSize: 14, fontWeight: 700, textDecoration: 'none', whiteSpace: 'nowrap', transition: 'background 250ms ease', flexShrink: 0 }}

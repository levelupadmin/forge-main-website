    ...
    <div className="overflow-hidden w-full">
      <div className={`flex items-center gap-12 md:gap-20 ${animClass}`} style={{ width: 'max-content' }}>
        {duplicated.map((item, i) => (
          <div key={`${item.name}-${i}`} className="flex-shrink-0 flex items-center justify-center" style={{ height: 48 }}>
            <img
              src={item.logo}
              alt={item.name}
              className="h-8 md:h-10 w-auto object-contain opacity-50 grayscale brightness-200"
              loading="lazy"
            />
          </div>
        ))}
